...
对于 Codex、Claude Code 或其他本地 agent，建议从仓库根目录开始，并要求 agent 先读取项目说明：

```text
打开 D:\AirMosaicAI\airmosaic-ai-core。
先读取 AGENTS.md。如果你是 Claude Code，也读取 CLAUDE.md。
使用 `airmosaic list-datasets` 查看已注册数据集。
...
*正在开发中。* `src/airmosaic_core/services/` 服务层提供 Python SDK 和 CLI，供 Codex、Claude Code 或自定义 agent 调用，用于数据发现、本地缓存解析和本地因果设计模板生成。MCP 工具定义和 REST API 仍在规划中。
...
AirMosaic AI 被设计为可由 Codex、Claude Code 等外部 agent 读取和调用。详见 [AGENTS.md](AGENTS.md)，其中包含 agent 安装、skill 发现、数据边界和操作约定。
...
Codex 应从仓库根目录读取 `AGENTS.md`。Claude Code 应读取 `CLAUDE.md`，该文件会指向同一份 agent 操作指南。每个 skill 通过 `agents/openai.yaml` 注册接口；运行 workflow 前应读取对应的 `SKILL.md`。
